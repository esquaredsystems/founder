...
- **Lists inside one cell** are semicolon-separated: `influence mining; complex
  networks`
...
**Keep each folder flat**, meaning files sit directly inside it, no subfolders. They
...
> Jekyll stops treating it as a post at all. It is skipped, with no error and
> no warning, and the piece silently vanishes. A collection has no such rule, so
...
Poetry posts get a centred composition: eyebrow, title and poem share one axis.
There are exactly two classes to remember: `.verse` wraps the poem and
...
by margin. Keeping those independent is what matters. If both gaps look the
same, the poem reads as a flat list of lines instead of a sequence of
ashaar.
...
in the front matter, not in the body. It is set apart from the poem automatically.
...
In `assets/css/style.css`, the first ~60 lines are design tokens:
